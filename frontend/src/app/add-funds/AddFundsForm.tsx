...
function AddFundsContent() {
  ...
  const handleDeposit = async () => {
    if (!displayAmount || displayAmount < 100) { toast.error('El monto mínimo es $100 ARS'); return; }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-dark-300">
      ...
      <div className="pt-24 pb-16">
        <div className="max-w-lg mx-auto px-4 sm:px-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="glass-card p-5 mb-4 flex items-center gap-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-5 sm:p-6 mb-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" min={100} value={custom} onChange={(e) => { setCustom(e.target.value); setAmount(0); }} placeholder="Ej: 3500" className="input-field pl-7" />
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="glass-card p-5 sm:p-6">
            ...
            <button onClick={handleDeposit} disabled={loading || !displayAmount || displayAmount < 100} className="btn-primary w-full flex items-center justify-center gap-2 py-4 text-base disabled:opacity-50">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><CreditCard className="w-5 h-5" /> Pagar con MercadoPago</>}
            </button>
