...
export default function ReferidosPage() {
  ...
  useEffect(() => {
    ...
    // Fetch fresh user to ensure referral_code is present
    authApi
      .getMe()
      .then((res) => {
        ...
        if (freshUser?.referral_code) {
          setReferralUrl(
            `${window.location.origin}/register?ref=${freshUser.referral_code}`,
          );
        }
      })
      .catch(() => {
        if (stored?.referral_code) {
          setReferralUrl(
            `${window.location.origin}/register?ref=${stored.referral_code}`,
          );
        }
      });
              ...
              <div>
                ...
                <p className="text-slate-400 mt-2 text-sm">
                  ...
                  <span className="text-primary-400 font-semibold">
                    {formatCurrency(summary?.rewardAmount ?? 0)}
                  </span>{" "}
                  por cada uno que gaste{" "}
                  <span className="text-white font-semibold">
                    {formatCurrency(summary?.spendThreshold ?? 0)}
                  </span>{" "}
                  o más.
                </p>
              </div>
                ...
                <button
                  onClick={() => {
                    if (navigator.share) {
                      navigator.share({
                        title: "FollowArg",
                        text: "¡Crecé en redes con FollowArg!",
                        url: referralUrl,
                      });
                    } else {
                      copyLink();
              ...
              {[
                {
                  ...
                  desc: "Enviá el link a amigos, clientes o seguidores. También podés compartirlo en tus redes.",
                },
                {
                  n: "2",
                  title: "Se registran gratis",
                  desc: "Tu contacto crea una cuenta sin costo usando tu link o código de referido.",
                },
                {
                  n: "3",
                  title: "Ganás cuando compran",
                  desc: `Cuando gastan ${formatCurrency(summary?.spendThreshold ?? 0)} o más, recibís ${formatCurrency(summary?.rewardAmount ?? 0)} en tu saldo automáticamente.`,
                },
              ].map((s) => (
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-600 text-xs mt-1 mb-5">
                  Compartí tu link y empezá a ganar hoy
                </p>
                <button
                  onClick={copyLink}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex justify-between text-[10px] text-slate-600">
                        ...
                        {ref.paid_at && (
                          <span>Recompensa: {formatDate(ref.paid_at)}</span>
                        )}
                        {!ref.paid_at && (
                          <span>Registrado: {formatDate(ref.created_at)}</span>
                        )}
                      </div>
                    </motion.div>
                  );
